...
export const getItems = () => dispatch => {//dispatch allows use to make a request
    ...
    axios
        .get(`${serverAddress}/api/items`)//makes the request to specified path in backend (from routes)
        .then(res =>
            dispatch({
                type: GET_ITEMS,
...
export const addItem = (item) => (dispatch, getState) => {
    axios
        .post(`${serverAddress}/api/items`, item, tokenConfig(getState))//passes item (obtained from onSubmit from itemModal) to api/items in router
        .then(res =>
            dispatch({
                type: ADD_ITEM,
...
export const deleteItem = (id) => (dispatch, getState) => {
    axios
        .delete(`${serverAddress}/api/items/${id}`, tokenConfig(getState))//attaches the token in header
        .then(res =>
            dispatch({
                type: DELETE_ITEM,
                payload: id
